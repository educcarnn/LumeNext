    ...
    <div className="bg-white shadow-md rounded-lg w-full md:w-3/4 lg:w-1/2 p-5">
        ...
        <main className="w-3/4 float-right">
            <header className="mb-5">
                {climaAtual && (
                    <h1 className="text-3xl font-bold text-blue-700">{climaAtual.location.name}</h1>
                )}
            </header>
            {climaAtual ? (
                <>
                    <p className="text-lg mb-2">
                        Temperatura Atual: <span className="font-semibold text-blue-500">{climaAtual.current.temp_c}°C</span>
                    </p>
                    <p className="text-lg mb-5">
                        Condição Atual: <span className="font-semibold text-blue-500">{climaAtual.current.condition.text}</span>
                    </p>
                    <h2 className="text-2xl text-blue-600 mb-3">Previsão para os próximos dias:</h2>
                    {previsao &&
                        previsao.map((dia, index) => (
                            <article key={index} className="mb-3 p-3 border rounded bg-blue-50">
                                <p className="text-lg">
                                    {dia.date}: <span className="font-semibold text-blue-400">{dia.day.avgtemp_c}°C, {dia.day.condition.text}</span>
                                </p>
                            </article>
                        ))}
                </>
